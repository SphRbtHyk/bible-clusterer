...
class DataBaseFiller:
    ...
    def __init__(self, mongo_uri: str = "", mongo_database: str = "greek", mongo_host: str = "localhost", mongo_port: int = 27017, mongo_user: str = "", mongo_password: str = "") -> None:
        """
        ...
        for file in Path(Path(__file__).resolve().parent / input_folder).glob("*.txt"):
            ...
            for line in split_text:
                if line:
                    word = line.split(" ")[-1]
                    text += word + " "
            self.texts.append({"book": book, "text": text, "language": "greek"})
            ...
            for text in split_text:
                ...
                if parsed_text[0]:
                    ...
                    try:
                        ...
                        texts[chapter_ix] = parsed_text[-1] + " "
            self.texts_chapter.append({"book": book, "chapters": texts, "language": "greek"})
            ...
            for texts in split_text:
                ...
                if parsed_text[0]:
                    ...
                    try:
                        ...
                        text[chapter_ix][verse_ix] = parsed_text[-1] + " "  
            self.texts_verses.append({"book": book, "verses": text, "language": "greek"})
            ...
            for verses_nbr, verses in opened_file.items():
                ...
                for verse_content in verses:
                    try:
                        ...
                        text[chapter_ix] = verse_content["lemma"] + " "
            self.texts_chapter.append({"book": book, "chapters": text, "language": "greek"})
    ...
    def load_vulgata(self, input_folder: str = "../data/vulgata/") -> None:
        """
        Load the Vulgata JSON files into a list of Python dictionary using encoding
        adapted to the text. The only loaded text is the lemmed and stemmed
        words, as only these will be considered whenever performing the
        clustering.
        ...
        Args:
            input_folder (str): Folder to find the data in
        """
        for file in Path(Path(__file__).resolve().parent / input_folder).glob("*.json"):
            logger.info(f"--- Writting down text found in files: {file} ---")
            book = file.name.split(".")[0]
            opened_file = json.loads(file.read_text("utf-8"))
            text = ""
            for _, verses in opened_file.items():
                for words in verses:
                    text += words["lemma"] + " "
            self.texts.append({"book": book, "text": text, "language": "latin"})

    def load_vulgata_chapters(self, input_folder: str = "../data/vulgata/") -> None:
        """
        Load the Vulgata JSON files into a list of Python dictionary using encoding
        adapted to the text. The only loaded text is the lemmed and stemmed
        words, as only these will be considered whenever performing the
            ...
            for verses_nbr, verses in opened_file.items():
                ...
                for verse_content in verses:
                    try:
                        ...
                        text[chapter_ix] = verse_content["lemma"] + " "
            self.texts_chapter.append({"book": book, "chapters": text, "language": "latin"})

    def load_vulgata_verses(self, input_folder: str = "../data/vulgata/") -> None:
        """
        Load the Vulgata JSON files into a list of Python dictionary on a per verse basis,
        using encoding adapted to the text. The only loaded text is the
        lemmed and stemmed words, as only these will be considered whenever performing the
        clustering.
            ...
            for verses_nbr, verses in opened_file.items():
                ...
                for verse_content in verses:
                    ...
                    try:
                        ...
                        text[chapter_ix][verse_ix] = verse_content["lemma"] + " "
            self.texts_verses.append({"book": book, "verses": text, "language": "latin"})
    ...
    def load_json(self):
        ...
        self.load_vulgata()
        self.load_vulgata_chapters()
        self.load_vulgata_verses()
    ...
    def write_booklist(self) -> None:
        ...
        self.database_instance.write_book_lists(
            [{"book": text["book"], "language": text["language"]} for text in self.texts])
...
def fill():
    ...
    """
    MONGO_URI = os.environ["CLUSTERER_MONGODB_URI"] if "CLUSTERER_MONGODB_URI" in os.environ else ""
    MONGO_HOST = os.environ["CLUSTERER_MONGODB_HOST"] if "CLUSTERER_MONGODB_HOST" in os.environ else "localhost"
    MONGO_PORT = os.environ["CLUSTERER_MONGODB_PORT"] if "CLUSTERER_MONGODB_PORT" in os.environ else 27017
    MONGO_DATABASE = os.environ["CLUSTERER_MONGODB_DATABASE"] if "CLUSTERER_MONGODB_DATABASE" in os.environ else "bible"
    MONGO_PASSWORD = os.environ["CLUSTERER_MONGODB_PASSWORD"] if "CLUSTERER_MONGODB_PASSWORD" in os.environ else None
    MONGO_USER = os.environ["CLUSTERER_MONGODB_USER"] if "CLUSTERER_MONGODB_USER" in os.environ else None
